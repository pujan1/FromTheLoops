// Dummy seed_dummy/active report fixtures so the wedge page has data to render.
// Density is deliberately mixed (see SEED_CELLS) to exercise both the rich and
// sparse views. Deterministic (fixed-seed PRNG) and idempotent (drops prior
// seed_dummy reports first).
...
// Deterministic PRNG (mulberry32) + choice helpers.
...
// Eight stable seed authors (upserted on clerk_id) so reports have FK targets.
...
// Density plan: (company, role, level) → report count, mixed dense/medium/sparse.
// Levels must match a curated company_levels.name (an unknown level throws).
...
export const SEED_CELLS: SeedCell[] = [
  ...
  // Medium (5–9) — broadens to role scope.
...
// Topic pools keyed by role family so questions get plausible tags.
...
// Canonical loop order; a report takes the first N rounds (N = 3..6).
...
// Inserts dummy reports per SEED_CELLS. Emits no outbox events — the runner
// backfills aggregates wholesale afterward.
export async function seedReports(db: Db): Promise<SeedReportsResult> {
  ...
  // Resolve taxonomy ids referenced by the plan.
  ...
  // Idempotency: drop prior seed_dummy reports (children CASCADE).
  await db.delete(interviewReports).where(eq(interviewReports.source, "seed_dummy"));

  let reportCount = 0;
  let roundCount = 0;
  let questionCount = 0;
  const REF = Date.UTC(2026, 4, 1); // fixed ref date for reproducibility
      ...
      await db.transaction(async (tx) => {
        ...
        for (let r = 0; r < nRounds; r++) {
          ...
          for (let q = 0; q < nQ; q++) {
            ...
            // ≥1 topic per question (submission invariant) + 1–2 extras.
