...
- 2026-06-04: **Days 5–8 done** (Typesense schemas + provisioning; indexer worker job + backfill; sparse-data fallback; `/admin/health`). db tests 109 → **118** (new `search-index.test.ts`); core tests 24 → **31** (new `scope.test.ts`); shared/db/core green; full repo typecheck + worker build/lint clean; `drizzle-kit generate` no pending diff (no schema changes this slice). Verified live against the local docker stack: provisioned + backfilled Typesense (31 companies, 93 topics), and a synthetic active report round-tripped report-write → event → `indexReportForEvent` → Typesense doc → full-text hit; worker boots clean (boot-provisions collections, LISTEN fans to both consumers, both sweeps fire).
  - **Day 5 — `@fromtheloop/search` is now real** (was a Sprint-0 stub). `src/client.ts` `getSearchClient()` (singleton, mirrors `getDb()`); `src/env.ts` reads `TYPESENSE_*` (defaults to local docker); `src/schemas/{reports,companies,topics}.ts` are the **committed collection schemas** (deliverable). `reports` carries the full-text `text` field + every facet the Day-4 sprint named (company/role/level/round-type/tags/outcome/trust-tier) + `created_at` default sort; `companies`/`topics` are name+aliases search ranked by `report_count`/`question_count`. `src/provision.ts` `ensureCollections()` is idempotent create-if-missing (never auto-drops — a schema change is a deliberate migration, out of V1). Provision two ways: `pnpm search:provision` (one-shot) **and** the worker self-provisions on boot (like it upserts BullMQ schedulers), so a fresh Hetzner deploy stands the collections up with no manual step.
  - **Day 5 — infra.** Dev `docker-compose.yml` Typesense healthcheck was permanently *unhealthy* (the `typesense:27.1` image ships no `wget`/`curl`) — swapped to the same bash `/dev/tcp` `/health` probe the prod compose already uses; **recreated the dev container and confirmed it goes healthy**. Prod worker (`infra/hetzner/docker-compose.prod.yml`) gained `TYPESENSE_HOST/PORT/PROTOCOL/API_KEY` env + a `depends_on: typesense (healthy)` so the indexer can reach the in-network container.
  - **Day 6 — indexer.** `reports` is the only **event-driven** collection in V1 (the events outbox only carries report writes). The search consumer mirrors the aggregate one exactly: db gained the `search_processed_at` drain trio (`claimUnprocessedSearchEvents` / `markSearchEventProcessed` / `countUnprocessedSearchEvents`) on the partial index reserved in Day 3–4. The testable core `indexReportForEvent(db, client, eventId)` lives in **search** (it needs both db + Typesense): load event → `deleted` op (or report no longer visible) drops the doc, `created`/`updated` upserts it → mark drained. **Idempotent** (missing event no-op, upsert = create-or-replace, delete tolerates 404), so retries + the racing sweep are safe. **Visibility filter is identical to the aggregate pipeline's** — `getReportForIndex` (new `packages/db/src/search-index.ts`) returns the denormalised doc shape only for `status='active' AND deleted_at IS NULL`, else null → indexer drops any stale doc, so a pending/deleted report can never leak into search (unit-tested).
  - **Day 6 — worker.** New `index-typesense` BullMQ queue, twin of `refresh-aggregate` (per-event `jobId=idx:<id>` + repeatable 30s sweep). `listen.ts` generalised: one LISTEN connection now fans **every** report-write NOTIFY out to **both** consumers (each with its own queue/jobId/opts). Worker `index.ts` ensures collections on boot (best-effort — a Typesense blip logs + continues, the sweep/retries recover) and closes the new queue on shutdown. **Worker package.json gained `typesense`** because the esbuild bundle inlines `@fromtheloop/search` but externalises `typesense` → it must resolve from the worker's own node_modules in prod.
  - **Day 6 — backfill.** `companies`/`topics` aren't event-driven; the backfill repopulates all three wholesale (`report_count`/`question_count` computed over **visible** reports only; reconciliation to keep counts fresh is a Sprint 6 job). `pnpm backfill:typesense` (provisions then indexes reports+companies+topics, batched upsert-by-id so re-runs overwrite not duplicate) and `pnpm backfill:aggregates` (wraps `refreshAllAggregates`) wired at repo root.
  - **Day 7 — sparse-data fallback.** `packages/core/src/aggregation/scope.ts` `decideScope(counts, threshold=10)` — a **pure** function (no DB/IO, easy to unit-test, which is the point of isolating it). Broadening ladder narrowest→widest: `exact` (the company·role·level cell, ≥10 reports → render directly) → `role` (same company+role, all levels) → `tag` (topic-tag aggregation, the never-empty floor). Returns `{scope, count, broadened}`. Exit criterion met: `'exact'` at ≥10, broadens below 10. PLAN.md §V1: "broaden scope with banner + tag-level aggregation when <10 reports per cell."
  - **Day 8 — `/admin/health`.** Server component (`apps/web/app/admin/health/`, `force-dynamic`) showing the three operational numbers: queue depth (`countUnprocessed{Aggregate,Search}Events`), recently-refreshed cells (new `listRecentAggregateRefreshes` + `countAggregateCells` in `aggregates.ts`, joined to company/role names), and per-collection Typesense doc counts (`collectionDocCounts`, degrades to 0 if Typesense is down). **Admin gating decision (reversible):** no RBAC exists in V1 (`users.role` deferred), so `/admin/*` is gated by an `ADMIN_CLERK_IDS` env allowlist — `middleware.ts` requires a session for `/admin(.*)`, then `requireAdmin()` (`lib/admin.ts`) `notFound()`s non-allowlisted ids (**404, not 403** — doesn't advertise the route). Swap for a real role check later. `ADMIN_CLERK_IDS` documented in `.env.example`; it's a **Vercel/web** env (not the Hetzner worker).
  - **⚠️ Worker redeploy needed (manual).** New worker code (index-typesense queue/sweep, two-consumer LISTEN, boot provisioning) **and** new env — the prod compose now requires `TYPESENSE_API_KEY` (already in `.env.prod.example`) for the worker service. Rebuild + re-ship the hand-shipped image to Hetzner, with `TYPESENSE_API_KEY` set in the box's `.env.prod`, or the indexer won't run. Also set `ADMIN_CLERK_IDS` in Vercel for `/admin/health` to be reachable.
  - **Deferred to remaining Sprint 3 days:** end-to-end submit→matview→Typesense latency measurement on the box (Day 9); ADR-0003 (Day 10) — will document the table-not-matview decision **and** the search-projection boundary + the admin-allowlist stopgap. Company/topic incremental indexing + count reconciliation deferred to Sprint 6 with the events-log prune.
