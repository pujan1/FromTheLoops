...
- [x] Cost estimate verified: ~$5/mo (Hetzner) + free tiers — checked on each provider's dashboard
...
- 2026-05-29: Day 10 — **cost exit criterion verified on provider dashboards: ~$5/mo holds.** Hetzner is the only recurring infra line (`cax11` ~€4.30 + primary IPv4 ~€0.50 ≈ €4.80/mo, no backups enabled); Neon, Clerk, Vercel (Hobby), Sentry (Developer), Cloudflare (DNS + Email Routing + R2 under free tier), and Resend all on free plans with $0 / no overage. Self-hosted Redis + Typesense add no SaaS cost (they ride on the one Hetzner box). Separate from the "~$5/mo infra": the `pujan.tech` domain is an annual registrar cost (~$30–50/yr for `.tech`), not a monthly line. **All 8 Sprint 0 exit criteria now met.**
